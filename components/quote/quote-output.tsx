  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-h-[88vh] max-w-2xl overflow-y-auto">
        ...
        {/* document */}
        <div className="space-y-4 rounded-lg border bg-white p-6 text-sm text-slate-800">
          <div className="flex items-start justify-between border-b pb-3">
            <div>
              <div className="text-lg font-semibold">Quote {p.quoteId}</div>
              <div className="text-xs text-slate-500">Issued {fmtDate(new Date().toISOString())} · Valid through {fmtDate(p.validTo)}</div>
            </div>
            <div className="flex items-center gap-2"><LogoMark className="size-9" /><div className="text-right text-xs leading-tight"><div className="font-semibold">Atlantic Project Cargo</div><div className="text-slate-500">Rate Management</div></div></div>
          </div>
          ...
          {!p.allInOnly ? (
            <>
              ...
              <table className="w-full border-t text-xs">
                <thead className="text-slate-500"><tr><th className="py-1 text-left">Service</th><th className="py-1 text-right">Amount</th></tr></thead>
                <tbody>
                  {included.map((leg) => (
                    <tr key={leg.id} className="border-t border-slate-100">
              ...
              <p className="text-[10px] text-slate-400">Surcharges are consolidated into each service line. Full carrier breakdown is internal to Atlantic Project Cargo.</p>
            </>
          ) : (
            <p className="rounded-md bg-slate-50 p-3 text-xs text-slate-500">All-in price only — itemized breakdown hidden for the client.</p>
          )}
